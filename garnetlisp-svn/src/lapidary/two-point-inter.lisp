...
;;; domain.                                                         ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; $Id: two-point-inter.lisp 114 2017-03-23 17:07:47Z rotgut $
;; 
...
;; accessor functions for various aggregates in the two-point
;; interactor menu


(defmacro TWO-POINT-START-WHERE ()
  `(g-value TWO-POINT-INTER-DIALOG :start-where))
(defmacro TWO-POINT-GRAY-SHADE1 ()
  `(g-value TWO-POINT-INTER-DIALOG :non-line-parameters :contents :gray-shade))
(defmacro TWO-POINT-FEEDBACK-OBJ ()
  `(g-value TWO-POINT-INTER-DIALOG :feedback-obj))
...
(defun TWO-POINT-INTERACTOR-NAME-FN (gadget interactor-name)
  (declare (special two-point-inter-dialog))
...
;;          (s-value (TWO-POINT-GRAY-SHADE1) :visible t) or
            (s-value (g-value TWO-POINT-INTER-DIALOG :non-line-parameters 
                                             :contents :gray-shade) :visible t)
            (s-value (g-value TWO-POINT-INTER-DIALOG :line-parameters 
...
(defun TWO-POINT-FEEDBACK-OBJ-FN (feedback-obj-box button-label)
  (declare (special *selection-info* TWO-POINT-INTER-DIALOG))
...
(defun FEEDBACK-OBJ-STANDARD (button button-label)
  (declare (special two-point-inter-dialog))
  (declare (ignore button))
  (s-value (g-value TWO-POINT-INTER-DIALOG :feedback-obj) :value button-label)
  (s-value (g-value TWO-POINT-INTER-DIALOG :feedback-obj) :field-string nil)
...
(defun FINAL-FUNCTION-PROTOTYPE (button button-label)
  (declare (special two-point-inter-dialog))
  (declare (ignore button))
  (s-value (g-value TWO-POINT-INTER-DIALOG :final-function :contents :final-function-box) 
                    :value nil)
...
(defun two-point-inter-do-go ()
  (let ((kr::*constants-disabled* nil))
    ...
    (create-instance 'TWO-POINT-INTER-DIALOG opal:aggregadget
	   ...
	   (:parts
		...
		(:parts
		 ((:title ,opal:text
		  ...
		  (:may-flip-over ,garnet-gadgets:motif-check-button
		     (:constant (t))
		     (:left ,(o-formula (+ 15 (gvl :parent :parent :left))))
		     (:top  ,(o-formula (+ 5  (opal:gv-bottom
	 ...
	 (:abort-if-too-small ,garnet-gadgets:motif-radio-button-panel
	    (:constant (t))
	    ...
	    (:parts
		 ...
		 (:parts
		  ((:feedback-obj-box ,SELECT-BOX
		   ...
		   (:standard-feedback ,garnet-gadgets:motif-radio-button
		      (:constant (t))
    ...
    (opal::fix-update-slots (g-value two-point-inter-dialog :start-where :contents
				     :select-box-panel))

    (create-instance 'TWO-POINT-INTER-WIN inter:interactor-window
      (:title "two point interactor")
      (:queue '*two-point-inter-queue*)
      (:aggregate two-point-inter-dialog)
      (:left 950)
      (:top 125)
      (:width (o-formula (+ (g-value two-point-inter-dialog :width) 30) 500))
      (:height (o-formula (+ (g-value two-point-inter-dialog :height) 30) 500)))
