...
;;; domain.                                                         ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; $Id: box.lisp 114 2017-03-23 17:07:47Z rotgut $
;;;
...
;; *box-constraint-menu* and box-constraint-win are referenced in
;; other files.
...
(defun box-constraint-do-go ()
  ...
  (create-instance 'BOX-CONSTRAINT-WIN inter:interactor-window
    ...
    (:left #-apple 880 #+apple 0)
    (:top #-apple 20 #+apple 200)
    (:width 490)
    (:height 560)
    )
  ...
  (create-instance '*BOX-CONSTRAINT-MENU* opal:aggregadget
    ...
    (:parts
     `((:title ,opal:text
       ...
       (:ok ,garnet-gadgets:motif-text-button-panel
	    (:constant (t))
	    (:button-width 44)
       ...
       ;;*******************************************************************
       ;; The LEFT-SLOT-AGG component is the set of gadgets associated
       ;; with the :left slot of the primary selection, and appears at
       ;; the top of the menu.
       ;; *******************************************************************
       ...
       
       ;;*******************************************************************
       ;; The TOP-SLOT-AGG component is the set of gadgets associated
       ;; with the :top slot of the primary selection, and appears at
       ...
       
       ;;*******************************************************************
       ;; The WIDTH-BUTTON component is an aggregadget which allows the
       ;; user to constrain the :width of the primary selection to the
					  ...
					  (progn
						...
						(progn
						   ...
						   *box-constraint-menu*
						   :width :width-over
						   :width-difference
						   (aref *width-constraint-vector*
							 (g-value obj :value))
						   :width-scale))
						(deselect-constraint-button 
						 (g-value inter :operates-on :con-button))))))
				    (:start-where
				     ,(o-formula (list :in-box (gvl :operates-on
								    :con-button))))))))

       
       ;;*******************************************************************
       ;; The WIDTH-SLOT-AGG component is the set of gadgets associated
       ;; with the :width slot of the primary selection, and appears at
       ...
       
       ;;*******************************************************************
       ;; The HEIGHT-BUTTON component is an aggregadget which allows
       ;; the user to constrain the :height of the primary selection to
       ;; the :height of the secondary selection.
       ;;*******************************************************************
	  ...
	  (:interactors
		     ...
		     (progn
			   ...
			   (progn
			      ...
			      *box-constraint-menu*
			      :height :height-over
			      :height-difference
			      (aref *height-constraint-vector*
				    (g-value obj :value))
			      :height-scale))
			   (deselect-constraint-button 
			    (g-value inter :operates-on :con-button))))))
	       (:start-where ,(o-formula (list :in-box
					       (gvl :operates-on :con-button))))))))

       
       ;;*******************************************************************
       ;; The HEIGHT-SLOT-AGG component is the set of gadgets
       ;; associated with the :height slot of the primary selection,
       ;; and appears at the bottom-right of the menu.
       ;;*******************************************************************
       ...
       ;;       selection, with a tiny rectangle at each of its four
       ;;       corners.
       ;;   2 LEFT-BUTTON-PANEL: the five buttons-with-arrows
       ;;       associated with the :left slot of the primary
       ;;       selection, which appear at the top of the menu
       ;;   3 TOP-BUTTON-PANEL: the five buttons-with-arrows associated
       ;;       with the :top slot of the primary selection, which
       ;;       appear at the left of the menu
       ;;*******************************************************************
