...
;;; $Id: truetype.lisp 2 2013-06-11 04:51:27Z rotgut $
;;
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (setf *Fixed-Font-Family* "Liberation Mono")
  (setf *Serif-Font-Family* "Times New Roman")
  (setf *Sans-Serif-Font-Family* "Liberation Sans"))
...
(create-instance 'opal::font opal:graphic-quality
  ...
  (:units-conversion-x 1)       ; Factor to convert from raw font units to pixels in the x direction.
  (:units-conversion-y 1)       ; Factor to convert from raw font units to pixels in the y direction.
...
(define-method :initialize font (font)
;;    ((instance font) &rest initargs &key family subfamily &allow-other-keys)
  (let ((family (g-value font :family))
	(subfamily (g-value font :subfamily)))
    (check-valid-font-families family subfamily)
    (s-value (gv font :units-conversion-x) (font-units->pixels-x (gv font :xfont)))
    (s-value (gv font :units-conversion-y) (font-units->pixels-y (gv font :xfont)))
    ))


(defun check-valid-font-families (family subfamily)
  (let ((fam (gethash family xft::*font-cache*)))
    (if (and fam (gethash subfamily fam))
	t
	(error "Font is not found: ~A ~A" family subfamily))))

(defun lookup-truetype-font (family)
  (let ((font (gethash family xft::*font-cache*)))
    (when font
      (format t "~A~%" font)
...
(defmacro with-font-loader ((loader font) &body body)
  (let ((exists-p (gensym))
        (font-path (gensym)))
    `(let ((,font-path (xft::get-font-pathname ,font)))
       (multiple-value-bind (,loader ,exists-p)
           (gethash ,font-path xft::*font-loader-cache*)
         (unless ,exists-p
           (setf ,loader (setf (gethash ,font-path xft::*font-loader-cache*)
...
(defun font-ascent (drawable font)
  ...
  (with-font-loader (loader font)
    (ceiling (* (gv font :units-conversion-y) (zpb-ttf:ascender loader)))))

(defun font-descent (drawable font)
  (declare (ignore drawable))
  "Returns ascent of @var{font}. @var{drawable} must be window, pixmap or screen."
  (with-font-loader (loader font)
    (floor (* (gv font :units-conversion-y) (zpb-ttf:descender loader)))))

(defun font-line-gap (drawable font)
  (declare (ignore drawable))
  "Returns ascent of @var{font}. @var{drawable} must be window, pixmap or screen."
  (with-font-loader (loader font)
    (ceiling (* (gv font :units-conversion-y) (zpb-ttf:line-gap loader)))))
 ...
 Text bounding box is only for contours. Bounding box for space (#x20) is zero."
  ...
  (multiple-value-bind (dpi-x dpi-y)
    ...
    (multiple-value-bind (string-bboxes exists-p)
        (gethash (cons dpi-x dpi-y) (gv font :string-bboxes) )
      (unless exists-p
	(setf string-bboxes 
	      (setf (gethash (cons dpi-x dpi-y) (gv font :string-bboxes))
