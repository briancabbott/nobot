...
;;; $Id: aggparam.lisp 114 2017-03-23 17:07:47Z rotgut $
...
to access the :items slot."
  (let* ((comp          (first components))
    ...
    (loop
       (when (member obj components) (return sel2a))
       (push :PARENT sel2a)
...
"Update primary selection prototype to use formulas on the named slots
 ...
 (let* ((selected   (first (g-value lapidary::*selection-info* :p-selected)) )
	(obj        selected)
   ...
   (loop
      (when (member obj components) (return))
